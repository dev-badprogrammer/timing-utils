...
public final class TimingStatistics {
     ...
     *
     * <p>Private — construction is routed through {@link #of} to keep instance creation centralized in one place,
     * consistent with the rest of the library's factory-method pattern.</p>
     *
     ...
     *
     * @return string containing all key metrics in milliseconds, plus iteration counts
     */
    @Override
    public String toString() {
        final var prefix    = "TimingStatistics[";
        final var delimiter = ", ";
        final var suffix    = "]";
        final var result    = new StringJoiner(delimiter, prefix, suffix);

        result.add("totalIterations=%d".formatted(getTotalIterations()));
        result.add("successfulIterations=%d".formatted(getSuccessfulIterations()));
        result.add("failedIterations=%d".formatted(getFailedIterations()));
        result.add("totalMillis=%dms".formatted(getTotalMillis()));
        result.add("averageMillis=%.3fms".formatted(getAverageMillis()));
        result.add("minMillis=%dms".formatted(getMinMillis()));
        result.add("maxMillis=%dms".formatted(getMaxMillis()));

        // Failure details — only when failures occurred
        if (hasFailures()) {
            getLastException().ifPresent(e -> result.add("lastException=%s".formatted(e)));
        }
